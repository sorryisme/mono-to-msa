...
threshold 숫자(p95 1000/1500/3000ms 등)는 첫 자동화 실행을 위한 임시값이다.

반복 횟수는 합격 조건이 아니다.

- 단순 실행·기능 확인: **1회**.
- 오류나 편차가 의심될 때: 같은 조건으로 한두 번 더 실행해 대조한다.
- 성능 개선 전후 비교, threshold 조정: 같은 조건에서 2~3회 정도를 탐색적으로 측정한다.

소수 반복으로 SLO 나 작은 성능 차이를 입증했다고 쓰지 않는다.
여기서 말하는 반복은 환경을 다시 만들어 전체를 재실행하는 것이다. 한 실행 안에서 서로 다른 경쟁 결과를 보는 `order-race --rounds` 와는 목적이 다르므로 함께 줄이지 않는다.
